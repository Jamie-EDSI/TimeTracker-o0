...
// Update the interface to accept clients data
...
// Update the component signature and use shared data
export function ActiveClientsReport({ onBack, onViewClient, clients = [] }: ActiveClientsReportProps) {
  ...
  // Replace the mockActiveClients with data derived from the passed clients
  const mockActiveClients =
    clients.length > 0
      ? clients.map((client) => ({
          ...
          caoNumber: client.caoNumber || "",
          ec: client.responsibleEC || "Lewis, Jillian",
          caseManager: client.caseManager || "",
          enrollmentDate: client.enrollmentDate,
          daysEnrolled: Math.floor(
            (new Date().getTime() - new Date(client.enrollmentDate).getTime()) / (1000 * 60 * 60 * 24),
          ),
          program: client.program,
          activity: "99 - Generic_Activity",
          activityStartDate: client.enrollmentDate,
          expectedHours: Number.parseInt(client.requiredHours) || 10,
          employmentStart: "",
          employmentEnd: "",
          claimedPlacement: "",
          status: client.status,
          phone: client.phone || client.cellPhone,
          email: client.email,
        }))
      : [
          {
