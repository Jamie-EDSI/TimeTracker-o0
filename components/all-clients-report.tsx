...
import { ArrowLeft, Search, Download, FileText, Filter, Eye, Users, Calendar, Clock, UserCheck } from "lucide-react"
...
export function AllClientsReport({ onBack, clients, onViewClient }: AllClientsReportProps) {
  ...
  const statuses = [...new Set(clients.map((client) => client.status))].sort()
  ...
  const handleExportExcel = async () => {
    setIsExporting(true)
    try {
      await exportToExcel(filteredClients, "All_Clients_Report")
    } catch (error) {
  ...
  const handleExportPDF = async () => {
    setIsExporting(true)
    try {
      await exportToPDF(filteredClients, "All Clients Report", "all-clients")
    } catch (error) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600">
                {filteredClients.length} of {totalClients} total clients
              </p>
            </div>
          </div>
